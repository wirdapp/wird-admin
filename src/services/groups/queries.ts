...
export const groupKeys = {
  ...
  list: (contestId: string | undefined) =>
    [...groupKeys.lists(), { contestId }] as const,
...
export function useGroupMembers(
  ...
  return useQuery({
    ...
    queryFn: () =>
      GroupsService.getGroupMembers({ groupId: groupId!, contestId: cid }),
    enabled: !!cid && !!groupId,
    ...options,
  });
}
...
export function useGroupLeaderboard(
  groupId: string | undefined,
  contestId?: string,
  options: Omit<UseQueryOptions<LeaderboardEntry[], Error>, 'queryKey' | 'queryFn' | 'enabled'> = {}
) {
  const cid = contestId ? String(contestId) : getCurrentContestId();
  return useQuery({
    queryKey: groupKeys.leaderboard(cid, groupId ?? ''),
    queryFn: () =>
      GroupsService.leaderboard({ groupId: groupId!, contestId: cid }),
    enabled: !!cid && !!groupId,
    ...options,
  });
}
...
export function useCreateGroup() {
  ...
  return useMutation({
    mutationFn: (body: GroupCreateData) =>
      GroupsService.createGroup({ body, contestId }),
    onSuccess: () => {
